...
class PhoneHomePlugin(base.BaseCloudConfigPlugin):
    ...
    def process(self, data, service):
        ...
        phone_home_data = None
        if phone_home_post_body:
            phone_home_data = json.dumps(phone_home_post_body)
        LOG.info("Calling home to url %s with data %r", phone_home_url,
                 phone_home_data)
        try:
            action = lambda: service._http_request(
                phone_home_url, phone_home_data)

...
class BaseOpenStackService(base.BaseMetadataService):
    ...
    def _parse_l3_network_data(self, network_data, network_l2_config):
        ...
        for network in network_data['networks']:
            ...
            parsed_network['id'] = network.get('network_id')
            parsed_network['name'] = network.get('id')
            if network.get('type'):
            ...
            if network.get('ip_address'):
                ip_address_prefix = network['ip_address'].split('/')
            ...
            if network.get('netmask'):
                parsed_network['netmask'] = network['netmask']
            if network.get('routes'):
                parsed_network['routes'] = network['routes']
                route_gateway = ([route for route in network['routes']
    ...
    def _parse_l4_network_data(self, network_data):
        ...
        for service in network_data['services']:
            if service.get('type') == 'dns':
                l4_config['global_dns_nameservers'].append(service['address'])
        return l4_config
